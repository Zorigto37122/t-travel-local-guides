...
function PhotoGallery({ photos, title }) {
  ...
  // Фото[0] — всегда соло.
  // Фото[1] — соло только если портрет (ratio < 1.0), иначе в паре с фото[2].
  // Остальные — строго парами.
  const cols = [[0]];
  let startIdx = 1;
  if (shown.length > 1) {
    const r1 = getRatio(1);
    if (r1 < 1.0 && shown.length > 1) {
      cols.push([1]);        // портрет → соло
      startIdx = 2;
    } else if (shown.length > 2) {
      cols.push([1, 2]);     // ландшафт → пара с фото[2]
      startIdx = 3;
    } else {
      cols.push([1]);
      startIdx = 2;
  ...
  // Ширина колонки в px = ratio × H (натуральные пропорции, без искажений)
  const colPx = (idxs) => {
          ...
          <div className="details-intro-body">
            <div className="details-intro-text">
              ...
              <div className="details-rating-row">
                {avgRating != null && (
                  <span
                    className="details-rating-badge"
        ...
        <div className="details-content">
          ...
          <div className="details-sidebar">
            ...
            <div className="details-booking" id="booking-anchor">
              ...
              {bookingMessage && (
                <div className="booking-success-message"><p>✓ {bookingMessage}</p></div>
              )}
              {bookingError && (
                <div className="booking-error-message"><p>✗ {bookingError}</p></div>
              )}
            </div>
          </div>
        </div>
